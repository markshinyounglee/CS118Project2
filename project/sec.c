...
ssize_t input_sec(uint8_t* buf, size_t max_length) {
    ...
    case SERVER_SERVER_HELLO_SEND: {
        ...
        // already packaged in TLV format

        // nonce signature 
        uint8_t* nonce_signature = (int8_t*) malloc(255); // usually signature is around 70-72 bytes long
        ...
        int total_size = nonce_size + cert_size + signature_size;
        uint8_t* load = (uint8_t*) malloc(total_size);
        memcpy(load, nonce_buf, nonce_size);
        memcpy(load+nonce_size, certificate, cert_size);
        memcpy(load+nonce_size+cert_size, nonce_sign_buf, signature_size);
...
void output_sec(uint8_t* buf, size_t length) {
    ...
    case SERVER_CLIENT_HELLO_AWAIT: {
        ...
        int nonce_loc = find_location(buf, state_sec, NONCE_CLIENT_HELLO);
    ...
    case CLIENT_SERVER_HELLO_AWAIT: {
        ...
        int peer_nonce_sign_loc = find_location(buf, state_sec, NONCE_SIGNATURE_SERVER_HELLO);
        ...
        int server_nonce_loc = find_location(buf, state_sec, NONCE_SERVER_HELLO);
        memcpy(peer_nonce, &buf[server_nonce_loc], NONCE_SIZE);
    ...
    case SERVER_KEY_EXCHANGE_REQUEST_AWAIT: {
        ...
        status = verify(
            nonce,  // raw nonce of the server
            NONCE_SIZE, 
            &buf[client_signed_nonce_loc],  // client-signed nonce
            client_signed_nonce_size,
            ec_peer_public_key // authority: client public key
        );
        if (status != 1)
        {
...
int find_location(uint8_t* data, int state_sec, uint8_t type)
{
    ...
    while(1)
    {
        ...
        if(*locator == type)
        {
            return currlen+3; // return the beginning index of field 
        }
        else if (total_size != 0 && currlen >= total_size) // maximum exceeded
        {
            break;
        }
        else if(*locator == SERVER_HELLO 
    ...
    fprintf(stderr, "Finding location failed for %d in %x\n", state_sec, type);
    exit(4);
}
